...
def get_default_configuration(network, task, network_trainer, plans_identifier=default_plans_identifier,
    ...
    student_task_name = 'ct_' + task
    teacher_task_name = 'pretrained_' + task
    pretrained_directory = join(network_training_output_dir, network, teacher_task_name, \
                            network_trainer.split('_')[0] + "__" + plans_identifier)

    if network == '2d':
        student_plans_file = join(preprocessing_output_dir, student_task_name, plans_identifier + "_plans_2D.pkl")
        plans_file = join(preprocessing_output_dir, task, plans_identifier + "_plans_2D.pkl")
    else:
        student_plans_file = join(preprocessing_output_dir, student_task_name, plans_identifier + "_plans_3D.pkl")
        plans_file = join(preprocessing_output_dir, task, plans_identifier + "_plans_3D.pkl")
    
    dataset_directory = join(preprocessing_output_dir, task)


    plans = load_pickle(student_plans_file)
    ...
    summarize_plans(student_plans_file)
    print("I am using stage %d from these plans" % stage)
    print('The output folder name is :', output_folder_name)
    ...
    return student_plans_file, output_folder_name, dataset_directory, batch_dice, \
        stage, pretrained_directory, plans_file, trainer_class
