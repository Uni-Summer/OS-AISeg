...
class feature_distillation(nn.Module):
    def __init__(self, num_pool):
        super(feature_distillation, self).__init__()
        self.l2 = nn.MSELoss(reduction='mean')
        self.sigmoid = nn.Sigmoid()
        self.act = F.tanh() # nn.LeakyReLU()
        self.num_pool = num_pool
        self.ssim_loss = SSIM(window_size=4, stride=4)
    
    def forward(self, ct_outputs, mct_outputs, mul_outputs):        
        ct_feats = ct_outputs[1::2]
        mct_feats = mct_outputs[1::2]
        mul_feats = mul_outputs[1::2]
        loss = 0.0
        
        # ssim_loss
        for j in range(len(ct_feats)):
            ct_feat = ct_feats[j]
            mct_feat = mct_feats[j]
            diff = ct_feat - mct_feat
            add = self.act(diff)
            out_feat = ct_feat * add
            loss += (1 - self.ssim_loss(out_feat, mul_feats[j]))

        avg_loss = loss / len(ct_feats)
        return avg_loss