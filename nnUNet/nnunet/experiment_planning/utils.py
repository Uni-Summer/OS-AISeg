...
def create_lists_from_splitted_dataset(base_folder_splitted):
    lists = []
    ###'/home/summer/nnUNetFrame/DATASET/nnUNet_raw/nnUNet_raw_data/Task800_Stroke'
    ...
    for tr in training_files:
        ...
        for mod in range(num_modalities):
            cur_pat.append(join(base_folder_splitted, "imagesTr", tr['image'].split("/")[-1][:-7] +
                                "_%04.0d.nii.gz" % mod))#'imagesTr/stroke_082_0000.nii.gz'
        cur_pat.append(join(base_folder_splitted, "labelsTr", tr['label'].split("/")[-1]))#labelsTr/stroke_082.nii.gz'
...
def crop(task_string, override=False, num_threads=default_num_threads):
    ...
    ##/home/summer/nnUNetFrame/DATASET/nnUNet_raw/nnUNet_cropped_data/Task800_Stroke
    if override and isdir(cropped_out_dir):
        shutil.rmtree(cropped_out_dir)
        maybe_mkdir_p(cropped_out_dir)

    splitted_4d_output_dir_task = join(nnUNet_raw_data, task_string)#'/home/summer/nnUNetFrame/DATASET/nnUNet_raw/nnUNet_raw_data/Task800_Stroke'
