...
class ExperimentPlanner(object):
    ...
    def plan_experiment(self):
        ...
        ##重采样Resample 
        # 重采样目的是解决在一些三维医学图像数据集中，不同的图像中单个体素voxel所代表的实际空间大小spacing不一致的问题
        # 因为卷积神经网络只在体素空间中进行操作，会忽视掉实际物理空间中大小信息。
        # 为了避免这种差异性，需要对不同图像数据在体素空间进行resize，保证不同的图像数据中，每个体素所代表的实际物理空间一致。
        # 

        # 具体需要将整个数据集resample到多大的spacing，即目标空间大小target_spacing应该多大呢？
        # nnUNet给出的建议是在大多数时候使用数据集各个图像不同spacing的中值，
        # 但是在各向异性（最大坐标上的spacing÷最小坐标上的spacing>3）的数据集中，
        # 取数据集10%分位点的spacing值作为spacing最大坐标的目标空间大小会是更好的选择。

        # 重采样的步骤可以简单分成3步。
        # 第一步是确定重采样的目标空间大小
        # 在之前数据格式转换的时候，每个数据的spacing信息存储在对应的pickle文件中，需要依次进行读取，
        # 然后一起存放在一个列表spacings当中。之后调用numpy中函数统计每个维度spacing的中值即可
        # # 接下来根据中值spacing进行判断，数据集是否存在各向异性的问题。
        # nnUNet设定的判断标准是，中值spacing中三个维度，是否有一个维度spacing大于另一个维度spacing的3倍，
        # 并且，该维度的中值size小于另一个维度中值size的1/3。
        # 如果存在各向异性，对spacing特别大的维度，取数据集中该维度spacing值的10%分位点作为该维度的目标空间大小。
        target_spacing = self.get_target_spacing()

        # 第二步根据target_spacing确定每张图像的目标尺寸。每张图像, spacing和shape之间的乘积为一个定值,代表整个图像在实际空间中的大小。
        # target shape * target spacing = original shape * original spacing
        # target shape = (original shape * original spacing) / target spacing
        new_shapes = [np.array(i) / target_spacing * np.array(j) for i, j in zip(spacings, sizes)]

        max_spacing_axis = np.argmax(target_spacing)
        remaining_axes = [i for i in list(range(3)) if i != max_spacing_axis]
        self.transpose_forward = [max_spacing_axis] + remaining_axes# [2, 0, 1]
        self.transpose_backward = [np.argwhere(np.array(self.transpose_forward) == i)[0][0] for i in range(3)]# [1, 2, 0]
    ...
    def determine_whether_to_use_mask_for_norm(self):
        ...
        for i in range(num_modalities):
            #用use_nonzero_mask指示是否只在nonzero区域进行normalization.
            if "CT" in modalities[i]:
                ...
                #如果在crop阶段图像大小减小了1/4以上的时候，nnUNet会选择仅在nonzero region进行normalization
        ...
        use_nonzero_mask_for_normalization = use_nonzero_mask_for_norm#OrderedDict([(0, False)])
    ...
    def run_preprocessing(self, num_threads):
        ...
        normalization_schemes = self.plans['normalization_schemes']# 0:'CT'
        use_nonzero_mask_for_normalization = self.plans['use_mask_for_norm']# 0:False
