...
class DatasetAnalyzer(object):
    def __init__(self, folder_with_cropped_data, overwrite=True, num_processes=default_num_threads):
        ...
        """
        self.num_processes = num_processes#8
        self.overwrite = overwrite#False
        self.folder_with_cropped_data = folder_with_cropped_data#/home/summer/nnUNetFrame/DATASET/nnUNet_raw/nnUNet_cropped_data/Task800_Stroke
        self.sizes = self.spacings = None
        self.patient_identifiers = get_patient_identifiers_from_cropped_files(self.folder_with_cropped_data)#[stroke_001,stroke_002,...]
    ...
    def get_size_reduction_by_cropping(self):
        ...
        for p in self.patient_identifiers:
            ...
            size_red = np.prod(shape_after_crop) / np.prod(shape_before_crop)#np.prod返回给定轴上的数组元素的乘积
            size_reduction[p] = size_red # 得到crop区域占全部数据的比例
        return size_reduction

    def _get_voxels_in_foreground(self, patient_identifier, modality_id):
        all_data = np.load(join(self.folder_with_cropped_data, patient_identifier) + ".npz")['data']#[2,8,256,256]
        modality = all_data[modality_id]#读取第id个模态的图像信息
        mask = all_data[-1] > 0## 生成前景mask
        #nnUNet为了简便计算，对每张图像仅采样1/10的前景体素用于统计，存储在voxels列表中。
    ...
    def _compute_stats(voxels):
        ...
        median = np.median(voxels)#统计数组元素的中位数
        mean = np.mean(voxels)#统计数组元素的平均值
        sd = np.std(voxels)#统计数组元素的方差
        mn = np.min(voxels)
        mx = np.max(voxels)
        #CT图像中经常会有异常大的孤立值和异常小的孤立值，需要先将图像HU值clip到前景HU值[0.5, 99.5]百分比范围之间
    ...
    def collect_intensity_properties(self, num_modalities):
        if self.overwrite or not isfile(self.intensityproperties_file):
            ...
            for mod_id in range(num_modalities):
                ...
                #第一步是收集整个CT影像训练集前景的统计信息。
                v = p.starmap(self._get_voxels_in_foreground, zip(self.patient_identifiers,
                                                              [mod_id] * len(self.patient_identifiers)))
                #通过对训练集中每张训练数据的遍历，将采样到的前景体素列表voxels拼接在一块，命名为w
                w = []
                for iv in v:
                    w += iv
                # 第二步调用numpy中的函数统计整个训练集前景的均值，标准差，0.5%分位HU值，99.5%分位HU值，
                # 并利用这些统计信息对每张图像进行clip以及z-scoring。代码中用use_nonzero_mask指示是否只在nonzero区域进行normalization.
                median, mean, sd, mn, mx, percentile_99_5, percentile_00_5 = self._compute_stats(w)
                #以防万一，计算并保存每张非CT三维图像自身的均值和标准差等信息
    ...
    def analyze_dataset(self, collect_intensityproperties=True):
        ...
        all_classes = [int(i) for i in classes.keys() if int(i) > 0]#1--除了背景，只有一个类别

        # modalities
        modalities = self.get_modalities()#0:'CT'

        # collect intensity information
        if collect_intensityproperties:#if include CT:
